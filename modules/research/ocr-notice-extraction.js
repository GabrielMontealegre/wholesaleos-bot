...
async function renderPdfPagesToPngs(pdfBuffer, caps, options = {}) {
  ...
  try {
    ...
    const dataUrls = await page.evaluate(async ({ pdfBase64, workerCode, maxPages, scale }) => {
...
async function runOcrNoticeExtraction(input = {}, options = {}) {
  ...
  for (const doc of documents) {
    ...
    try {
      const pngs = await renderPdfPagesToPngs(doc.buffer, caps, options);
      let text = '';
      let confidence = 0;
      ...
      const docRows = txTrusteeNoticeExtractor.extractTrusteeNoticeRows(text, {
      ...
      }).map((row) => tagOcrRow(row, { confidence }));
      diagnostics.ocr_documents_succeeded += 1;
      diagnostics.ocr_rows_extracted += docRows.length;
      diagnostics.ocr_rows_with_address += docRows.filter((row) => cleanText(row.address)).length;
      diagnostics.ocr_rows_with_sale_date += docRows.filter((row) => cleanText(row.sale_date)).length;
      attempts.push({ url, status: 'ocr_done', ms: Date.now() - started, confidence, rows: docRows.length, text_excerpt: cleanText(text).slice(0, 240) });
      for (const row of docRows) rows.push(row);
    } catch (error) {
