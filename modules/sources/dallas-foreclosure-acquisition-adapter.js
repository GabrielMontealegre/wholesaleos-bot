...
function normalizeSourceRecords(records) {
  return (Array.isArray(records) ? records : []).map((record, index) => {
    ...
    return {
      ...
      amount_or_judgment: cleanText(item.amount_or_judgment),
      tax_due: cleanText(item.tax_due || item.tax_amount),
      tax_due_evidence_text: cleanText(item.tax_due_evidence_text || item.tax_amount_evidence_text),
      mortgage_arrears: cleanText(item.mortgage_arrears),
      mortgage_arrears_evidence_text: cleanText(item.mortgage_arrears_evidence_text),
      lien_amount: cleanText(item.lien_amount || item.tax_lien_amount),
      lien_amount_evidence_text: cleanText(item.lien_amount_evidence_text || item.tax_lien_amount_evidence_text),
      judgment_amount: cleanText(item.judgment_amount),
      judgment_amount_evidence_text: cleanText(item.judgment_amount_evidence_text),
      redemption_amount: cleanText(item.redemption_amount || item.delinquent_redemption_amount),
      redemption_amount_evidence_text: cleanText(item.redemption_amount_evidence_text || item.delinquent_redemption_amount_evidence_text),
      minimum_bid: cleanText(item.minimum_bid),
      minimum_bid_evidence_text: cleanText(item.minimum_bid_evidence_text),
      opening_bid: cleanText(item.opening_bid),
      opening_bid_evidence_text: cleanText(item.opening_bid_evidence_text),
      assessed_value: cleanText(item.assessed_value || item.appraised_value),
      assessed_value_evidence_text: cleanText(item.assessed_value_evidence_text || item.appraised_value_evidence_text),
      public_estimate: cleanText(item.public_estimate || item.estimated_value),
      public_estimate_evidence_text: cleanText(item.public_estimate_evidence_text || item.estimated_value_evidence_text),
      listing_price: cleanText(item.listing_price || item.listed_price),
      listing_price_evidence_text: cleanText(item.listing_price_evidence_text || item.listed_price_evidence_text),
...
function buildCombinedSourceText(sourceText, sourceHtml, records) {
  ...
  for (const record of records) {
    pieces.push([
      ...
      record.tax_due,
      record.mortgage_arrears,
      record.lien_amount,
      record.judgment_amount,
      record.redemption_amount,
      record.minimum_bid,
      record.opening_bid,
      record.assessed_value,
      record.public_estimate,
...
function candidateFromRaw(rawCandidate, context, sourceMeta) {
  ...
  const baseCandidate = {
    ...
    mortgage_arrears: cleanText(raw.mortgage_arrears),
    mortgage_arrears_evidence_text: cleanText(raw.mortgage_arrears_evidence_text),
    lien_amount: cleanText(raw.lien_amount || raw.tax_lien_amount),
    lien_amount_evidence_text: cleanText(raw.lien_amount_evidence_text || raw.tax_lien_amount_evidence_text),
    judgment_amount: cleanText(raw.judgment_amount),
    judgment_amount_evidence_text: cleanText(raw.judgment_amount_evidence_text),
    redemption_amount: cleanText(raw.redemption_amount || raw.delinquent_redemption_amount),
    redemption_amount_evidence_text: cleanText(raw.redemption_amount_evidence_text || raw.delinquent_redemption_amount_evidence_text),
    minimum_bid: cleanText(raw.minimum_bid),
    minimum_bid_evidence_text: cleanText(raw.minimum_bid_evidence_text),
    opening_bid: cleanText(raw.opening_bid),
    opening_bid_evidence_text: cleanText(raw.opening_bid_evidence_text),
    assessed_value: cleanText(raw.assessed_value || raw.appraised_value),
    assessed_value_evidence_text: cleanText(raw.assessed_value_evidence_text || raw.appraised_value_evidence_text),
    public_estimate: cleanText(raw.public_estimate || raw.estimated_value),
    public_estimate_evidence_text: cleanText(raw.public_estimate_evidence_text || raw.estimated_value_evidence_text),
    listing_price: cleanText(raw.listing_price || raw.listed_price),
    listing_price_evidence_text: cleanText(raw.listing_price_evidence_text || raw.listed_price_evidence_text),
...
function mergeRecordIntoCandidate(candidate, records) {
  ...
  return propertyCandidate.normalizePropertyCandidate(Object.assign({}, candidate, {
    ...
    tax_due: matchingRecord.tax_due || candidate.tax_due,
    tax_due_evidence_text: matchingRecord.tax_due_evidence_text || candidate.tax_due_evidence_text,
    mortgage_arrears: matchingRecord.mortgage_arrears || candidate.mortgage_arrears,
    mortgage_arrears_evidence_text: matchingRecord.mortgage_arrears_evidence_text || candidate.mortgage_arrears_evidence_text,
    lien_amount: matchingRecord.lien_amount || candidate.lien_amount,
    lien_amount_evidence_text: matchingRecord.lien_amount_evidence_text || candidate.lien_amount_evidence_text,
    judgment_amount: matchingRecord.judgment_amount || candidate.judgment_amount,
    judgment_amount_evidence_text: matchingRecord.judgment_amount_evidence_text || candidate.judgment_amount_evidence_text,
    redemption_amount: matchingRecord.redemption_amount || candidate.redemption_amount,
    redemption_amount_evidence_text: matchingRecord.redemption_amount_evidence_text || candidate.redemption_amount_evidence_text,
    minimum_bid: matchingRecord.minimum_bid || candidate.minimum_bid,
    minimum_bid_evidence_text: matchingRecord.minimum_bid_evidence_text || candidate.minimum_bid_evidence_text,
    opening_bid: matchingRecord.opening_bid || candidate.opening_bid,
    opening_bid_evidence_text: matchingRecord.opening_bid_evidence_text || candidate.opening_bid_evidence_text,
    assessed_value: matchingRecord.assessed_value || candidate.assessed_value,
    assessed_value_evidence_text: matchingRecord.assessed_value_evidence_text || candidate.assessed_value_evidence_text,
    public_estimate: matchingRecord.public_estimate || candidate.public_estimate,
    public_estimate_evidence_text: matchingRecord.public_estimate_evidence_text || candidate.public_estimate_evidence_text,
    listing_price: matchingRecord.listing_price || candidate.listing_price,
    listing_price_evidence_text: matchingRecord.listing_price_evidence_text || candidate.listing_price_evidence_text,
...
async function runDallasForeclosureAcquisitionAdapter(options = {}) {
  ...
  const enrichedRawCandidates = rawCandidates.map((candidate) => {
    ...
    return Object.assign({}, candidate, matchingRecord ? {
      ...
      amount_or_judgment: matchingRecord.amount_or_judgment || candidate.amount_or_judgment,
      tax_due: matchingRecord.tax_due || candidate.tax_due || candidate.tax_amount,
      tax_due_evidence_text: matchingRecord.tax_due_evidence_text || candidate.tax_due_evidence_text || candidate.tax_amount_evidence_text,
      mortgage_arrears: matchingRecord.mortgage_arrears || candidate.mortgage_arrears,
      mortgage_arrears_evidence_text: matchingRecord.mortgage_arrears_evidence_text || candidate.mortgage_arrears_evidence_text,
      lien_amount: matchingRecord.lien_amount || candidate.lien_amount || candidate.tax_lien_amount,
      lien_amount_evidence_text: matchingRecord.lien_amount_evidence_text || candidate.lien_amount_evidence_text || candidate.tax_lien_amount_evidence_text,
      judgment_amount: matchingRecord.judgment_amount || candidate.judgment_amount,
      judgment_amount_evidence_text: matchingRecord.judgment_amount_evidence_text || candidate.judgment_amount_evidence_text,
      redemption_amount: matchingRecord.redemption_amount || candidate.redemption_amount || candidate.delinquent_redemption_amount,
      redemption_amount_evidence_text: matchingRecord.redemption_amount_evidence_text || candidate.redemption_amount_evidence_text || candidate.delinquent_redemption_amount_evidence_text,
      minimum_bid: matchingRecord.minimum_bid || candidate.minimum_bid,
      minimum_bid_evidence_text: matchingRecord.minimum_bid_evidence_text || candidate.minimum_bid_evidence_text,
      opening_bid: matchingRecord.opening_bid || candidate.opening_bid,
      opening_bid_evidence_text: matchingRecord.opening_bid_evidence_text || candidate.opening_bid_evidence_text,
      assessed_value: matchingRecord.assessed_value || candidate.assessed_value || candidate.appraised_value,
      assessed_value_evidence_text: matchingRecord.assessed_value_evidence_text || candidate.assessed_value_evidence_text || candidate.appraised_value_evidence_text,
      public_estimate: matchingRecord.public_estimate || candidate.public_estimate || candidate.estimated_value,
      public_estimate_evidence_text: matchingRecord.public_estimate_evidence_text || candidate.public_estimate_evidence_text || candidate.estimated_value_evidence_text,
      listing_price: matchingRecord.listing_price || candidate.listing_price || candidate.listed_price,
      listing_price_evidence_text: matchingRecord.listing_price_evidence_text || candidate.listing_price_evidence_text || candidate.listed_price_evidence_text,
