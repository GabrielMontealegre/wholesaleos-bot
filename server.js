// Deploy: 2026-04-30T11:53:02.497Z
...
// ============================================================
// COMP AGENT ROUTES v2 — safe load with lazy require
// ============================================================

// GET /api/leads/:id/comps
app.get('/api/leads/:id/comps', function(req, res) {
  var agent;
  try { agent = require('./modules/agents/comp-agent'); } catch(e) { return res.status(503).json({error:'comp-agent load error: '+e.message}); }
  agent.fetchCompsForLead(req.params.id)
    .then(function(r){ if(r.error) return res.status(404).json(r); res.json(r); })
    .catch(function(e){ logger.error('[comps] '+e.message); res.status(500).json({error:e.message}); });
});

// GET /api/leads/:id/seller-script
app.get('/api/leads/:id/seller-script', function(req, res) {
  var agent;
  try { agent = require('./modules/agents/seller-script-agent'); } catch(e) { return res.status(503).json({error:'seller-script-agent load error: '+e.message}); }
  agent.generateSellerScript(req.params.id)
    .then(function(r){ res.json(r); })
    .catch(function(e){ logger.error('[seller-script] '+e.message); res.status(500).json({error:e.message}); });
});

// POST /api/leads/search-fresh
app.post('/api/leads/search-fresh', function(req, res) {
  ...
  var dealEngine;
  try { dealEngine = require('./modules/deal-engine'); } catch(e) { return res.status(503).json({error:'deal-engine load error'}); }
  Promise.resolve(dealEngine.runDailyIngestion(state, count))
    .then(function(n){ res.json({ok:true,state:state,requested:count,inserted:n||0}); })
    .catch(function(e){ logger.error('[search-fresh] '+e.message); res.status(500).json({error:e.message}); });
});

// POST /api/buyers/:id/send-leads — no address
app.post('/api/buyers/:id/send-leads', function(req, res) {
  try {
    var dbData = db.readDB();
    var buyer = (dbData.buyers||[]).filter(function(b){return b.id===req.params.id;})[0];
    if(!buyer) return res.status(404).json({error:'Buyer not found'});
    ...
    leads.forEach(function(l,i){
      ...
      lines.push((i+1)+'. '+(l.city||'')+', '+(l.state||'')+' - '+(l.lead_type||'SFR')+'\n'+
        '   ARV: '+arv+' | Asking: '+mao+' | Spread: '+spread+' | Fee: '+fee+'\n'+
        '   Source: '+(l.source||'Public Record')+'\n'+
        '   [Address withheld - provided upon signed contract]\n');
    });
    lines.push('\nInterested? Reply with the number and I will send full details.');
    res.json({ok:true,buyer_id:buyer.id,buyer_name:buyer.name,lead_count:leads.length,message:lines.join('\n')});
  } catch(e) {
...
// Schedule daily comp batch at 4AM UTC (uses existing cron from top of file)
cron.schedule('0 4 * * *', function() {
  try {
    var ca = require('./modules/agents/comp-agent');
    ca.runDailyCompBatch().catch(function(e){console.error('[comp-batch] '+e.message);});
  } catch(e) { console.error('[comp-batch-cron] load error: '+e.message); }
}, {timezone:'UTC'});
