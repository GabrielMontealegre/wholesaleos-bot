...
// ============================================================
// DAILY DEAL INGESTION — runs at 6AM server time
// Pulls fresh leads from all 9 sources across 14 target states
// Uses existing dealEngine + dedup + db.addLead pipeline
// ============================================================
const INGESTION_STATES = [
  'NY','IL','TX','CA','FL','GA','NC','AZ','WA','MA','LA','PA','OH','MI'
];

async function runDailyIngestion() {
  console.log('[Daily Ingestion] Started at', new Date().toISOString());
  var totalInserted = 0;
  var totalSkipped  = 0;

  for (var i = 0; i < INGESTION_STATES.length; i++) {
    var state = INGESTION_STATES[i];
    console.log('[Daily Ingestion] Processing state:', state);

    try {
      var rawDeals = await dealEngine(state, 50);
      var { analyzeProperty } = require('./ai');
      var inserted = [];
      var skipped  = [];

      for (var j = 0; j < rawDeals.length; j++) {
        var d = rawDeals[j];

        // Build lead using same structure as playwright route
        var leadInput = {
          address:           (d.address || '').trim(),
          city:              (d.city    || '').trim(),
          state:             (d.state   || state).trim(),
          zip:               d.zip      || '',
          source:            d.source   || 'Deal Engine',
          source_platform:   d.source   || 'Deal Engine',
          status:            'New Lead',
        ...
        if (!leadInput.address || leadInput.address === 'Unknown Address') {
          skipped.push(leadInput.address);
          continue;
        }

        // Skip exact same-source duplicate
        var isDup = db.getLeads().some(function(l) {
          return (l.address||'').toLowerCase().trim() === leadInput.address.toLowerCase() &&
                 (l.city||'').toLowerCase().trim()    === leadInput.city.toLowerCase() &&
                 (l.state||'').toLowerCase().trim()   === leadInput.state.toLowerCase() &&
                 (l.source||'').toLowerCase().trim()  === leadInput.source.toLowerCase();
        });
        if (isDup) { skipped.push(leadInput.address); continue; }

        // Increment sourceCount on any existing lead at same address+city+state
        var existingMatch = db.getLeads().find(function(l) {
          return (l.address||'').toLowerCase().trim() === leadInput.address.toLowerCase() &&
                 (l.city||'').toLowerCase().trim()    === leadInput.city.toLowerCase() &&
                 (l.state||'').toLowerCase().trim()   === leadInput.state.toLowerCase();
        ...
        if (existingMatch) {
          db.updateLead(existingMatch.id, { sourceCount: (existingMatch.sourceCount || 1) + 1 });
        }

        // AI scoring — same as route, never blocks insertion
        try {
          var enriched = await analyzeProperty(leadInput);
          Object.assign(leadInput, enriched);
        } catch(aiErr) {
          console.warn('[Daily Ingestion] AI skip:', leadInput.address, aiErr.message);
        }

        db.addLead(leadInput);
        inserted.push(leadInput.address);
      }

      console.log('[Daily Ingestion] Inserted:', inserted.length, '| Skipped:', skipped.length, '| State:', state);
      totalInserted += inserted.length;
      totalSkipped  += skipped.length;

    } catch(stateErr) {
      console.error('[Daily Ingestion] Error on state', state, ':', stateErr.message);
    }
  }

  console.log('[Daily Ingestion] Complete. Total inserted:', totalInserted, '| Total skipped:', totalSkipped);
}

// Schedule: 0 6 * * * = 6AM server time (UTC)
cron.schedule('0 6 * * *', function() {
  console.log('[Daily Ingestion] Cron triggered');
  runDailyIngestion().catch(function(e) {
    console.error('[Daily Ingestion] Cron error:', e.message);
  });
}, { timezone: 'UTC' });
