...
| Public county ArcGIS / parcel APIs | Public government data | Owner-of-record, owner mailing address, and disclosure-state sold sales where counties publish parcel/sales layers | Low: public endpoints only; no login/CAPTCHA bypass | Zero (plain HTTPS GET) | ADOPT WHERE VERIFIED — 2026-08-11 discovery: San Diego owner/mailing fields open, Detroit public sales open, Bexar parcel API failed and still failed on 30-second retry, so it stays caveated/unverified |
