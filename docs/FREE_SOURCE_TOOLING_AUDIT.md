...
| Serper (existing provider) | Commercial (free tier in caps) | Property-specific listing discovery (Zillow /homedetails/, Redfin /home/, Realtor detail URLs) | Low within caps | Zero | MOTHBALL FROM DEFAULT QUEUE — bounded local evidence probe on 2026-07-14 used all 6 query groups but returned `provider_bad_response`, 0 reviewed, and 0 accepted property-specific URLs (`exports/listing-radar-evidence/2026-07-14.json`); Root cause (2026-07-14, offline code trace): free query mode strips site: operators (sanitizeSerperFreeQuery), so radar queries are rewritten into local-intent brand searches that cannot return /homedetails/-style property URLs; the >=3 property-specific gate is unpassable on the free Serper tier by construction. Radar stays mothballed indefinitely on the free tier; revisit only if Gabriel explicitly opts into a Serper tier permitting advanced query operators (paid decision). |
