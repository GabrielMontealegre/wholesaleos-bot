// ══ WholesaleOS MEGA PATCH — All 9 issues fixed ══

// ─── UTILITY: Full address builder ───
function fullAddress(l) {
  if (!l) return '';
  // address field often contains full address already
  var addr = l.address || '';
  // If address already has city/state/zip, use it
...
function zillowLink(l) {
  ...
  return 'https://www.zillow.com/homes/' + encodeURIComponent(fa) + '_rb/';
}
function redfinLink(l) {
  var fa = fullAddress(l);
  return 'https://www.redfin.com/search#location=' + encodeURIComponent(fa) + '&propertyType=house';
}
function rentometerLink(l) {
  var fa = fullAddress(l);
  return 'https://www.rentometer.com/analysis/new?address=' + encodeURIComponent(fa) + '&bedrooms=' + (l.beds||3);
}
function mapsLink(l) {
  var fa = fullAddress(l);
  return 'https://www.google.com/maps/search/' + encodeURIComponent(fa);
}

// ─── FIX 3: Universal modal X button helper ───
function modalX(closeFn) {
  return '<button onclick="' + closeFn + '" style="background:none;border:none;font-size:26px;cursor:pointer;color:#86868b;line-height:1;padding:0;flex-shrink:0">✕</button>';
}

// ─── FIX 4: Rename search placeholder ───
(function() {
  function fixSearchLabel() {
    document.querySelectorAll('input[placeholder*="Search leads"]').forEach(function(el) {
      el.placeholder = '🔍  Search Deals & Buyers...';
    });
    document.querySelectorAll('input[placeholder*="Search Deals"]').forEach(function(el) {
      if (!el.placeholder.includes('Buyers')) el.placeholder = '🔍  Search Deals & Buyers...';
    });
  }
  fixSearchLabel();
  setTimeout(fixSearchLabel, 1500);
  if (!window._searchLabelObs) {
    window._searchLabelObs = new MutationObserver(fixSearchLabel);
    window._searchLabelObs.observe(document.body, {childList:true, subtree:true});
  }
})();

// ─── FIX 5: Lead pagination — larger options ───
...
// ─── FIX 1+2+3: Lead detail modal with full address + working links + X button ───
function openLeadDetailFixed(leadId) {
  ...
  modal.innerHTML = '<div style="background:#fff;border-radius:18px;width:760px;max-width:97vw;max-height:92vh;overflow-y:auto;box-shadow:0 30px 80px rgba(0,0,0,.4);font-family:-apple-system,sans-serif">' +
    '<div style="padding:20px 24px;border-bottom:1px solid #f0f0f0;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;background:#fff;z-index:1;border-radius:18px 18px 0 0">' +
      '<div>' +
        '<div style="display:flex;align-items:center;gap:10px">' +
          ...
          '<span style="font-size:16px;font-weight:700">' + (l.type||'SFR') + ' · ' + (l.county||l.state||'') + '</span>' +
        ...
        '</div>' +
        '<div style="font-size:13px;color:#1a1a2e;font-weight:600;margin-top:4px">📍 ' + fa + '</div>' +
      '</div>' +
      '<button onclick="closeLeadOverlay()" style="background:none;border:none;font-size:28px;cursor:pointer;color:#86868b;line-height:1;padding:4px">✕</button>' +
    '</div>' +
    '<div style="padding:20px 24px">' +
      ...
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:18px;font-size:12px">' +
        '<div style="background:#f9f9fb;border-radius:10px;padding:12px">' +
          ...
          '<div>💰 List Price: ' + (l.list_price?'$'+Number(l.list_price).toLocaleString():'?') + ' · DOM: ' + (l.dom||'?') + '</div>' +
          '<div>⚠️ Distress: ' + (l.distress||'?') + '</div>' +
          '<div>🏷️ ' + (l.deal_classification||l.investment_strategy||'?') + '</div>' +
        '</div>' +
        '<div style="background:#f9f9fb;border-radius:10px;padding:12px">' +
          '<div style="font-weight:700;margin-bottom:8px;font-size:11px;color:#86868b">SELLER INFO</div>' +
          '<div>📞 ' + (l.phone||'No phone') + '</div>' +
          '<div>🏷️ Seller: ' + (l.seller_type||'?') + ' · Owner: ' + (l.ownership||'?') + '</div>' +
          '<div>📉 Reductions: ' + (l.reductions||0) + '</div>' +
          '<div>📅 Added: ' + (l.created||'?') + '</div>' +
          '<div style="margin-top:6px;font-size:11px;color:#86868b">Full Address for copy:</div>' +
          '<div style="font-weight:600;color:#1a1a2e;font-size:12px;cursor:pointer" onclick="navigator.clipboard.writeText(''+fa.replace(/'/g,"\'")+'')" title="Click to copy">📋 ' + fa + '</div>' +
        '</div>' +
      '</div>' +
      '<div style="margin-bottom:14px">' +
        '<div style="font-weight:700;font-size:12px;color:#86868b;margin-bottom:8px">PROPERTY LINKS (exact address)</div>' +
      ...
      '<div style="display:flex;gap:8px;margin-top:4px">' +
        '<select onchange="updateLeadStatus(''+leadId+'',this.value)" style="flex:1;padding:8px;border:1px solid #e5e5ea;border-radius:8px;font-size:12px">' +
          ['New Lead','Contacted','Follow-Up','Qualified','Not Interested','Dead','Archived'].map(function(s){return '<option value="'+s+'"'+(s===(l.status||'New Lead')?' selected':'')+'>'+s+'</option>';}).join('') +
        '</select>' +
...
function updateLeadStatus(leadId, status) {
  fetch('/api/leads/'+leadId, {method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:status})})
    ...
    }).catch(function(){toast('Status saved locally','');});
}

// Hook openLeadModal to use our enhanced version
var _origOpenLeadModal = typeof openLeadModal === 'function' ? openLeadModal : null;
openLeadModal = function(id) { openLeadDetailFixed(id); };

// ─── FIX 8: Outreach Hub — make clicks work ───
function renderOutreachHub() {
  var leads = APP.leads || [];
  var buyers = APP.buyers || [];
  var newLeads = leads.filter(function(l){return !l.status||l.status==='New Lead';}).slice(0,50);
  ...
  var leadRows = newLeads.map(function(l){
    var fa = fullAddress(l);
    var rid = getDealRefId(l);
    return '<div data-lid="'+l.id+'" onclick="openLeadDetailFixed(this.dataset.lid)" style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;border-bottom:1px solid #f5f5f7;cursor:pointer;transition:background .15s" onmouseover="this.style.background='#f9f9fb'" onmouseout="this.style.background=''">' +
      '<div>' +
        '<div style="display:flex;align-items:center;gap:8px">' +
          ...
          '<span style="font-size:13px;font-weight:600">'+(l.type||'SFR')+' · '+fa+'</span>' +
        '</div>' +
        '<div style="font-size:11px;color:#86868b;margin-top:2px">'+(l.phone||'No phone')+' · '+l.state+'</div>' +
      '</div>' +
  ...
  var buyerRows = buyers.map(function(b){
    var mc = leads.filter(function(l){return matchBuyersToLead(l,[b]).length>0;}).length;
    return '<div data-bid="'+b.id+'" onclick="openBulkSendToBuyer(this.dataset.bid)" style="display:flex;justify-content:space-between;align-items:center;padding:12px 16px;border-bottom:1px solid #f5f5f7;cursor:pointer;transition:background .15s" onmouseover="this.style.background='#f9f9fb'" onmouseout="this.style.background=''">' +
      '<div>' +
        '<div style="font-size:13px;font-weight:600">'+b.name+'</div>' +
        '<div style="font-size:11px;color:#86868b">'+(b.city||'')+' '+(b.state||'')+' · '+(b.email||'No email')+'</div>' +
      '</div>' +
      '<div style="text-align:right">' +
  ...
  return '<div style="max-width:1100px">' +
    ...
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">' +
      ...
      '</div>' +
    '</div>' +
    (contacted.length ? '<div style="margin-top:16px;background:#fff;border:1px solid #e5e5ea;border-radius:14px;overflow:hidden"><div style="padding:14px 16px;border-bottom:1px solid #f0f0f0;font-weight:700;font-size:14px">🔄 Follow-Up Queue ('+contacted.length+')</div><div style="max-height:200px;overflow-y:auto">'+contacted.map(function(l){var fa=fullAddress(l);var rid=getDealRefId(l);return '<div data-lid="'+l.id+'" onclick="openLeadDetailFixed(this.dataset.lid)" style="display:flex;justify-content:space-between;padding:10px 16px;border-bottom:1px solid #f5f5f7;cursor:pointer" onmouseover="this.style.background='#f9f9fb'" onmouseout="this.style.background=''"><div><div style="font-size:13px;font-weight:600">'+rid+' '+fa+'</div><div style="font-size:11px;color:#0071e3">'+l.status+'</div></div><div style="font-size:13px;font-weight:700;color:#34c759">$'+(l.spread||0).toLocaleString()+'</div></div>';}).join('')+'</div></div>' : '') +
  '</div>';
}

// Hook outreach into the render system
if (window.APP && !window._outreachHooked) {
  ...
  render = function() {
    if (APP.page === 'outreach') {
      ...
      if (el) el.innerHTML = renderOutreachHub();
      return;
    }
...
// ─── FIX 5: Override renderLeads with larger page options + full address ───
renderLeads = function() {
  ...
  // EXPANDED options: 200, 500, 1000, 2000, 5000, 10000
  ...
  var rows = pageLeads.map(function(l){
    ...
    return '<tr style="cursor:pointer;border-bottom:1px solid #f5f5f7" data-lid="'+l.id+'" onclick="openLeadDetailFixed(this.dataset.lid)">'+
      ...
      '<td style="padding:9px 8px;font-size:12px;max-width:300px">'+fa+'</td>'+
      ...
      '<td style="padding:9px 8px;font-size:11px;color:#0071e3">'+(mc?mc+' 👤':'')+'</td>'+
    '</tr>';
  }).join('');
  ...
  return '<div>'+
    ...
    '<span style="font-size:11px;color:#86868b;align-self:center">'+total.toLocaleString()+' total</span>'+
...
// ─── FIX 6: Email status check ───
function checkEmailStatus() {
  fetch('/api/email/status').then(function(r){return r.json();}).then(function(d){
    console.log('Email status:', JSON.stringify(d));
    toast(d.connected ? 'Email connected: '+d.email : 'Email not connected — check Gmail OAuth', d.connected?'success':'error');
  }).catch(function(e){
    toast('Email API not reachable — Gmail OAuth token may be expired', 'error');
    console.log('Email check error:', e.message);
  });
}

// ─── FIX 7: Auto daily scan ───
function scheduleDailyScan() {
  if (window._dailyScanTimer) clearInterval(window._dailyScanTimer);
  // Check every hour if 24h has passed since last scan
  window._dailyScanTimer = setInterval(function(){
    var last = parseInt(localStorage.getItem('lastAutoScan')||'0');
    var now = Date.now();
    if (now - last > 23*60*60*1000) { // 23h
      localStorage.setItem('lastAutoScan', String(now));
      ...
      fetch('/api/scan', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nationwide:true,limit:200})})
        .then(function(r){return r.json();}).then(function(d){
          toast((d.added||0)+' new leads added in daily scan', 'success');
          loadLeadsFromAPI();
        }).catch(function(){console.log('Daily scan failed');});
    }
  }, 60*60*1000); // check every hour
  console.log('Daily scan scheduler started');
}
scheduleDailyScan();

// ─── FIX BUYERS TAB with bulk send ───
renderBuyers = function(){
  ...
  var cards=buyers.map(function(b){
    ...
    return '<div style="background:#fff;border:1px solid #e5e5ea;border-radius:14px;padding:16px">'+
      ...
      '<div>✉️ '+(b.email?'<strong>'+b.email+'</strong>':'<span style="color:#ff9500;font-weight:600">No email</span> <button data-bid="'+b.id+'" onclick="openBuyerDetail(this.dataset.bid)" style="background:none;border:none;color:#0071e3;cursor:pointer;font-size:11px;padding:0">Add →</button>')+'</div>'+
      '</div>'+
      ...
      '<div style="color:'+(mc>0?'#0071e3':'#86868b')+'">🔎 '+mc+' deals match</div>'+
      '</div>'+
...
// ─── Bulk send + buyer finder (carried from v4) ───
function openBulkSendToBuyer(buyerId){
  ...
  modal.innerHTML='<div style="background:#fff;border-radius:16px;width:95vw;max-width:1100px;max-height:90vh;display:flex;flex-direction:column;box-shadow:0 30px 80px rgba(0,0,0,.4)">'+
    ...
    '</div></div>'+
    (!buyer.email?'<div style="background:#fff3cd;border:1px solid #ff9500;border-radius:8px;padding:10px 16px;font-size:12px;color:#8a6400;margin:0 24px 12px">⚠️ No email for '+buyer.name+' — edit buyer to add email before sending</div>':'')+
...
async function confirmBulkSendToBuyer(id){var b=(APP.buyers||[]).find(function(x){return x.id===id;});if(!b)return;if(!b.email){toast('Add email for '+b.name+' first','error');return;}var c=Array.from(document.querySelectorAll('#bulk-send-modal input[type=checkbox]:checked'));if(!c.length){toast('Select at least one deal','error');return;}var ds=c.map(function(x){return x.dataset.dealId;});toast('Sending '+ds.length+' deals...','');try{await fetch('/api/buyers/'+id+'/send-deals',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({dealIds:ds})});closeBulkModal();toast('Sent '+ds.length+' deals to '+b.name,'success');}catch(e){toast('Error: '+e.message,'error');}}
...
function populateLeadsForState(state){toast('Scanning leads for '+state+'...','');fetch('/api/scan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({states:[state],limit:50})}).then(function(r){return r.json();}).then(function(d){toast((d.added||0)+' new leads added for '+state,'success');loadLeadsFromAPI();}).catch(function(){toast('Scan queued for '+state,'');});}

// Matching + buyer finder carried over
function matchBuyersToLead(lead,buyers){buyers=buyers||APP.buyers||[];if(!lead||!buyers.length)return[];return buyers.map(function(b){var score=0,reasons=[];var bs=(b.states&&b.states.length)?b.states:(b.state?[b.state]:[]);if(bs.length&&bs.indexOf(lead.state)===-1)return{buyer:b,score:0,reasons:[]};score+=30;reasons.push('state');if(!(b.buyTypes||[]).length||(b.buyTypes||[]).indexOf(lead.type)>-1){score+=25;reasons.push('type');}var op=lead.offer||(lead.arv?Math.round(lead.arv*0.7):0);if(op>=(b.minPrice||0)&&op<=(b.maxPrice||999999999)){score+=25;reasons.push('price');}return{buyer:b,score:score,reasons:reasons};}).filter(function(m){return m.score>=30;}).sort(function(a,b){return b.score-a.score;});}

var _bfRunning=false;
async function runBuyerFinder(state,cities){if(_bfRunning){toast('Finder already running','');return;}_bfRunning=true;var el=document.getElementById('buyer-finder-status');var qs=['we buy houses','cash home buyers','real estate investors','wholesale real estate','fix and flip buyers'];var found=[],saved=0;for(var ci=0;ci<cities.length;ci++){var city=cities[ci];for(var qi=0;qi<qs.length;qi++){var q=qs[qi]+' '+city+' '+state;if(el)el.textContent='Searching: '+q+'...';try{var res=await fetch('/api/buyer-search?q='+encodeURIComponent(q)+'&state='+state+'&city='+encodeURIComponent(city));if(!res.ok)continue;var data=await res.json();(data.results||[]).forEach(function(s){var txt=(s.snippet||'')+' '+(s.title||'');if(!['cash','investor','wholesale','buyers list','we buy'].some(function(k){return txt.toLowerCase().includes(k);}))return;var ph=(txt.match(/\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/g)||[]);var em=(txt.match(/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/g)||[]);found.push({name:s.title||q,phone:ph[0]||'',email:em[0]||'',city:city,state:state,website:s.url||''});});}catch(e){}await new Promise(function(r){setTimeout(r,200);});}}var seen={},deduped=[];found.forEach(function(b){var k=b.phone||b.email||(b.website.split('/')[2]||b.name).replace('www.','');if(k&&!seen[k]){seen[k]=true;deduped.push(b);}});if(el)el.textContent='Saving '+deduped.length+' buyers...';for(var i=0;i<deduped.length;i++){try{var r=await fetch('/api/buyers',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:deduped[i].name,phone:deduped[i].phone,email:deduped[i].email,city:deduped[i].city,state:deduped[i].state,states:[deduped[i].state],buyTypes:['SFR','Multi'],website:deduped[i].website})});if(r.ok)saved++;}catch(e){}}
_bfRunning=false;if(typeof loadMatchingData==='function')await loadMatchingData();if(el)el.textContent='Done! Saved '+saved+' buyers for '+state;toast('Found '+saved+' buyers in '+state,'success');}
...
console.log('WholesaleOS MEGA PATCH loaded — all 9 issues addressed');
