...
async function main() {
  ...
  for (const n of ['07', '10', '17']) {
    ...
    const pages = await ocr.renderPdfPagesToPngs(fs.readFileSync(path.join(baseline, doc.body_file)), { ...ocr.DEFAULT_CAPS, max_pages_per_doc: 2, render_scale: 1.4 });
  ...
  fs.writeFileSync(path.join(proof, 'document-proof.json'), JSON.stringify({ default_ocr_runtime: runtime, outputs, inspection_runtime: 'Shared Playwright browser resolver; scale 1.4 is display-only and does not change extraction settings.' }, null, 2));
