...
(async () => {
  try {
    ...
    assert.strictEqual(enriched.canonical_opportunity_after_execution.packet_status, 'CONTACT_LOOKUP');
    assert.strictEqual(enrichedPacket.packet_status, 'CONTACT_LOOKUP');
    assert.strictEqual(enrichedPacket.contact.route_type, 'NONE');
    assert.strictEqual(enrichedPacket.contact.phone, '');
    ...
    assert.strictEqual(enriched.contacts_found.length, 0);
    ...
    assert.ok(enriched.updated_work_orders.some((item) => item.type === 'FIND_CONTACT_ROUTE'));
