...
(async () => {
  ...
  async function runBexarLiveStyleWithSeededLedger(ledgerName, seedEntry) {
    const baseLedgerPath = path.join(tmpDir, ledgerName);
    const ledgerMarketKey = `bexar-live-style-${ledgerName}`;
    const originalLedgerPath = process.env.DEAL_BOARD_DOCUMENT_LEDGER_PATH;
    process.env.DEAL_BOARD_DOCUMENT_LEDGER_PATH = baseLedgerPath;
    const ledgerPath = adapter.documentLedgerFilePath({ market_key: ledgerMarketKey });
    fs.writeFileSync(ledgerPath, JSON.stringify({
      version: 1,
      store_kind: 'deal_board_document_ledger_not_saved_leads',
      updated_at: '2026-07-29T00:00:00.000Z',
      documents: {
        [bexarLiveStyleLedgerKey]: seedEntry
      }
    }, null, 2));
    try {
      const first = await adapter.runTxCountyForeclosureAcquisitionAdapter({
        source_id: 'tx_bexar_county_foreclosure_notices',
        market_key: ledgerMarketKey,
        env: { ENABLE_SEARCH_PROVIDER: 'false' },
        fetch_impl: async (url) => {
      ...
      const repeat = await adapter.runTxCountyForeclosureAcquisitionAdapter({
        source_id: 'tx_bexar_county_foreclosure_notices',
        market_key: ledgerMarketKey,
        env: { ENABLE_SEARCH_PROVIDER: 'false' },
        fetch_impl: async (url) => {
      ...
      return { first, repeat, ledger: JSON.parse(fs.readFileSync(ledgerPath, 'utf8')) };
    } finally {
  ...
  for (const { first, repeat, ledger } of bexarSeededLedgerRuns) {
    ...
    assert.strictEqual(first.candidates.length, 3);
    assert.strictEqual(first.diagnostics.docs_ledger_skipped, 0, 'stale or missing parser signatures must force one re-read');
    assert.ok(first.candidates.some((candidate) => candidate.source_row_reference === '20260800237' && /13123 Laguna Del Rey Dr, Atascosa, TX 78002/i.test(candidate.normalized_address)));
    assert.ok(first.candidates.some((candidate) => candidate.source_row_reference === '20260800012' && /1803 Burnet St, San Antonio, TX 78202/i.test(candidate.normalized_address)));
    assert.ok(first.candidates.some((candidate) => candidate.source_row_reference === '20260800346' && !candidate.normalized_address && candidate.missing_evidence.includes('street number')));
